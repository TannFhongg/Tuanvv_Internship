...
### Bài tập: Tìm lỗi out-of-bounds bằng GDB

File thực hành: `out_of_bounds_debug.cpp`.

Chương trình có một lời gọi truy cập phần tử vượt khỏi phạm vi của `std::vector`. Hàm `at()` phát hiện lỗi, ném `std::out_of_range` và làm chương trình kết thúc vì exception không được xử lý.

#### Yêu cầu

1. Build chương trình với debug symbols và tắt tối ưu hóa.
2. Chạy chương trình để tái hiện lỗi.
3. Mở chương trình bằng GDB.
4. Đặt breakpoint tại `readValue`.
5. Kiểm tra `index`, nội dung vector và call stack.
6. Xác định hàm nào truyền index không hợp lệ.
7. Sửa nguyên nhân và chạy lại để xác nhận chương trình không còn crash.

#### Build và tái hiện lỗi

```powershell
$env:Path = "C:\msys64\ucrt64\bin;$env:Path"
g++ -std=c++20 -g -O0 -Wall -Wextra -Wpedantic .\out_of_bounds_debug.cpp -o .\out_of_bounds_debug.exe
.\out_of_bounds_debug.exe
```

#### Debug bằng GDB

```gdb
gdb ./out_of_bounds_debug.exe
break readValue
run
print index
print values
...
Tại breakpoint, hãy trả lời:

- `index` có giá trị bao nhiêu?
- Vector có bao nhiêu phần tử?
- Khoảng index hợp lệ là gì?
- Dòng nào trong `calculateTotal` truyền index sai?
- Call stack cho biết `readValue` được gọi qua những hàm nào?

Có thể yêu cầu GDB dừng ngay lúc exception được ném:
...
Build và chạy lại bằng cùng lệnh. Chương trình phải kết thúc bình thường, sau đó có thể kiểm tra thêm với sanitizer:

```powershell
g++ -std=c++20 -g -O1 -Wall -Wextra -fsanitize=address,undefined .\out_of_bounds_debug.cpp -o .\out_of_bounds_debug.exe
.\out_of_bounds_debug.exe
```
